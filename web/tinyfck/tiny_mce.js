...
_createIFrame:function(replace_element,doc,win){
    ...
    if(tinyMCE.isMSIE&&!tinyMCE.isOpera)replace_element.outerHTML=iframe.outerHTML;else replace_element.parentNode.replaceChild(iframe,replace_element);
    if(tinyMCE.isMSIE&&!tinyMCE.isOpera)return win.frames[id];else return iframe;
},
...
removeTinyMCEFormElements:function(form_obj){
    ...
    if(form_obj.nodeName!="FORM"){
        if(form_obj.form)form_obj=form_obj.form;else form_obj=tinyMCE.getParentElement(form_obj,"form");
    }
...
case"keyup":case"keydown":
    ...
    if(inst&&inst.handleShortcut(e))return false;
    if(e.target.editorId)tinyMCE.selectedInstance=tinyMCE.instances[e.target.editorId];else return;
    if(tinyMCE.selectedInstance)tinyMCE.selectedInstance.switchSettings();
    var inst=tinyMCE.selectedInstance;
    if(tinyMCE.isGecko&&tinyMCE.settings['force_p_newlines']&&(e.keyCode==8||e.keyCode==46)&&!e.shiftKey){
    if(TinyMCE_ForceParagraphs._handleBackSpace(tinyMCE.selectedInstance,e.type)){
...
break;
case"mousedown":case"mouseup":case"click":case"focus":
    tinyMCE.hideMenus();
    if(tinyMCE.selectedInstance){
...
openWindow:function(template,args){
    ...
    if(tinyMCE.isMSIE)height+=40;else height+=20;
...
switch(command){
    ...
    case"InsertUnorderedList":case"InsertOrderedList":
        ...
        if(tinyMCE.isSafari)this.execCommand("mceInsertContent",false,"<"+tag+"><li>&nbsp;</li><"+tag+">");else this.getDoc().execCommand(command,user_interface,value);
        tinyMCE.triggerNodeChange();
        break;
    case"Strikethrough":
        if(tinyMCE.isSafari)this.execCommand("mceInsertContent",false,"<strike>"+this.selection.getSelectedHTML()+"</strike>");else this.getDoc().execCommand(command,user_interface,value);
        tinyMCE.triggerNodeChange();
        break;
    case"mceSelectNode":
...
TinyMCE_Engine.prototype.cleanupHTMLCode=function(s){
    ...
    if(tinyMCE.isMSIE)s=s.replace(new RegExp('<p><hr \\/><\\/p>','gi'),"<hr>");
    if(tinyMCE.getParam('convert_urls'))s=s.replace(new RegExp('(href=\"{0,1})(\\s*#)','gi'),'$1'+tinyMCE.settings['document_base_url']+"#");
    return s;
};
...
TinyMCE_Engine.prototype._cleanupHTML=function(inst,doc,config,elm,visual,on_save,on_submit){
    ...
    c.sourceIndex=-1;
    if(s.cleanup_serializer=="xml")h=c.serializeNodeAsXML(elm);else h=c.serializeNodeAsHTML(elm);
...
if(this.isMSIE)return this.xmlDoc.xml;else return new XMLSerializer().serializeToString(this.xmlDoc);
},
...
xmlEncode:function(s){
    ...
    switch(this.settings.entity_encoding){
        ...
        case"named":
            ...
            c=s.charCodeAt(i);
            e=this.entities[c];
            if(e&&e!='')o+='&'+e+';';else o+=String.fromCharCode(c);
        }
        return o;
        case"numeric":
            for(i=0,l=s.length;i<l;i++){
            c=s.charCodeAt(i);
            if(c>127||c==60||c==62||c==38||c==39||c==34)o+='&#'+c+";";else o+=String.fromCharCode(c);
        }
        return o;
    }
    return s;
...
_getAttrib:function(e,n,d){
    ...
    if(this.isMSIE&&e.nodeName=="FORM"&&n=="enctype"&&v=="application/x-www-form-urlencoded")v="";
    if(this.isMSIE&&e.nodeName=="INPUT"&&n=="size"&&v=="20")v="";
    if(this.isMSIE&&e.nodeName=="INPUT"&&n=="maxlength"&&v=="2147483647")v="";
...
TinyMCE_Engine.prototype.clearArray=function(a){
    for(var k in a)a[k]=null;return a;
};
...
TinyMCE_Selection.prototype={
    getSelectedHTML:function(){
        ...
        if(tinyMCE.isGecko)e.appendChild(r.cloneContents());else e.innerHTML=r.item?r.item(0).outerHTML:r.htmlText;
